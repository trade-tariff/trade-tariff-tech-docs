RSpec.describe GitHubRepoFetcher do
  before :each do
    stub_request(:get, "https://api.github.com/users/trade-tariff/repos?per_page=100")
      .to_return(
  ...
  describe "#readme" do
    ...
    def readme_url
      "https://raw.githubusercontent.com/trade-tariff/#{repo_name}/main/README.md"
    end
  ...
  describe "#docs" do
    ...
    def docs_url(repo_name)
      "https://api.github.com/repos/trade-tariff/#{repo_name}/contents/docs"
    end
    ...
    context "the repo contains a reachable docs/ folder" do
      ...
      it "retrieves documents recursively" do
        ...
        allow(stubbed_client).to receive(:contents).with("trade-tariff/#{repo_name}", path: "docs")
          .and_return([dir])
        allow(stubbed_client).to receive(:contents).with("trade-tariff/#{repo_name}", path: "docs/foo")

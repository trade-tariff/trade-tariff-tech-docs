...
class ExternalDoc
  def self.parse(markdown, repository: "", path: "")
    context = {
      ...
      base_url: URI.join(
        "https://github.com",
        "trade-tariff/#{repository}/blob/main/",
      ),
      image_base_url: URI.join(
        "https://raw.githubusercontent.com",
        "trade-tariff/#{repository}/main/",
      ),
    }
    ...
    def call
      doc.search("a").each do |element|
        ...
        element["href"] = if href.start_with?("/")
                            ...
                            # github.com/trade-tariff/REPO_NAME/foo.txt.
                            ...
                            # not a `trade-tariff/REPO_NAME/bar.txt` result.
  ...
  private
    ...
    def internal_doc_name(repository, uri_path = "")
      internal_doc = uri_path.match(/^\/trade-tariff\/#{repository}\/blob\/(?:main|master)\/docs\/([^\/]+)\.md$/)
